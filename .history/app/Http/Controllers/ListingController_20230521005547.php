...
class ListingController extends Controller
{
    ...
    public function getProducts()
    {
        $products = DB::table('listings')
            ->selectRaw('listings.*, users.name, users.email, users.location, users.profile_picture, CONCAT("[", GROUP_CONCAT(JSON_OBJECT(offers.buyer_id, offers.price)), "]") AS offer_prices,CONCAT("[", GROUP_CONCAT(JSON_OBJECT(path,product_images.path)), "]") AS path')
        ...
        DB::raw('JSON_ARRAYAGG(JSON_OBJECT(path,product_images.path)) AS path');
        ...
        foreach ($response['products'] as $product) {
            ...
            $product->path = json_decode($product->offer_prices);
