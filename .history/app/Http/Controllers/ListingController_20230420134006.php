...
class ListingController extends Controller
{
    ...
    public function getProducts()
    {
        ...
        ->selectRaw('listings.*, users.name, users.email, users.location, users.profile_picture, CONCAT("[", GROUP_CONCAT(JSON_OBJECT("offers_price", offers.price)), "]") AS offer_prices')
        ...
        foreach ($products as $product) {
            $offer_prices = json_decode($product->offer_prices);
            // Access individual offer prices like so:
            $first_offer_price = $offer_prices[0]->offers_price;
            // ...
        }
