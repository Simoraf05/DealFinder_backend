...
class ListingController extends Controller
{
    public function AddProduct(Request $request)
    {
        ...
        if ($request->hasFile('path')) {
            foreach ($request->file('path') as $image) {
                ...
                $product_image->path = 'images/' . $imageName;
                $product_image->save();
            }
        }
        ...
        return response()->json(['message' => 'product added successfully', 'product' => $product, 'status' => 201]);
    }
    ...
    public function getProducts()
    {
        $products = DB::table('listings')
            ->selectRaw('listings.*, users.name, users.email, users.location, users.profile_picture, CONCAT("[", GROUP_CONCAT(JSON_OBJECT(offers.buyer_id, offers.price)), "]") AS offer_prices, GROUP_CONCAT(path) AS path')
    ...
    public function searchForProductByName(Request $request)
    {
        $resultat = DB::table('listings')
            ...
            ->groupBy('listings.id', 'listings.title', 'listings.description', 'listings.price', 'listings.shipping_options', 'listings.category', 'listings.status', 'listings.seller_id', 'listings.created_at', 'listings.updated_at')
            ->get();
