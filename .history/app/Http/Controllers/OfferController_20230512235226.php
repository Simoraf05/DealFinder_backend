...
class OfferController extends Controller
{
    ...
    public function getMyOffers(Request $request)
    {
        ...
        ->select('offers.id as offer_id', 'offers.price', 'offers.status', 'listings.title', 'listings.image', 'listings.price', 'listings.id as idP')
        ->join('listings', 'listings.id', '=', 'offers.listing_id')
        ->join('users', 'users.id', '=', 'offers.buyer_id')
        ->where('listing_id', '=', $request->seller_id)
        ->get();
