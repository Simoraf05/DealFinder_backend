...
class ListingController extends Controller
{
    public function AddProduct(Request $request)
    {
        $request->validate([
            ...
            'images.*' => 'required|mimes:jpeg,png,jpg', // Updated validation for multiple images
            'category' => 'required',
            'seller_id' => 'required',
        ]);
        ...
        if ($request->hasFile('images')) {
            ...
            $product_image->path = $product_image->store('products');
            $product_image->save();
            }
        }
            $imageName = time() . '_' . $image->getClientOriginalName();
            $image->move(public_path('images'),$imageName);
            $imageData = new Product_images();
            $imageData -> product_id = $product->id;
            $imageData -> path = 'images/' . $imageName;
            $imageData->save();
