...
class ListingController extends Controller
{
    ...
    {
        $categoryQuery = DB::select(DB::raw("SELECT DISTINCT category FROM listings"));
        $categoryValues = array_map(function ($category) {
            return $category->category;
        }, $categoryQuery);
        $categoryString = implode(",", $categoryValues);
        
        $listingQuery = DB::table('listings')
            ->whereRaw("category IN ($categoryString)")
            ->get();
        
        return response()->json(['listings' => $listingQuery]);
